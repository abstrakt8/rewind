...
import { API_URL } from "./backend/constants";
...
function* waitForBackendState(state: BackendState): SagaIterator {
  while (true) {
    const action = yield take(stateChanged.name);
...
function* watchForBackendReady(): SagaIterator {
  ...
  yield spawn(watchReplaysAdded, API_URL);
...
async function fetchStatus(): Promise<BackendState> {
  try {
    const status = await fetch(`${API_URL}/api/status`);
    const data = await status.json();
    return data.state as BackendState;
  } catch (err) {
